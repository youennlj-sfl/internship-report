...
Enfin, une fois que les conteneurs sont intégrés dans l'image SEAPATH, il faut aussi les utiliser dans notre détection de vulnérabilités car ils font partie de notre pile logicielle. Nous avons donc modifié le _hook_ FAI de la @fig:debian:syft:hook pour également scanner les images de conteneurs et générer des @SBOM:pl, qu'on exporte en même temps que celui du système de fichier. Le pipeline pour donner tous ces @SBOM:pl à Grype et à VulnScout a également été modifié afin d'avoir une vue des vulnérabilités la plus complète possible.

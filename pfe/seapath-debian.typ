...
Afin d'estimer si les vulnérabilités remontées étaient cohérentes, nous avons utilisé VulnScout qui est capable d'interpréter les fichiers de résultat de Grype via son outil de scan intégré. Nous avons pris avantage de cette capacité en faisant un léger changement dans le code (voir @chapter:vulnscout:seapath-analysis), ce qui nous a permis de voir toutes les vulnérabilités dans VulnScout. Nous avons survolé les plus importantes, et il ne semble pas y avoir de faux positifs : comme indiqué plus haut, Grype se sert directement du traqueur de sécurité de Debian, donc les données sont très précises.
...
Enfin, une fois que les conteneurs sont intégrés dans l'image SEAPATH, il faut aussi les utiliser dans notre détection de vulnérabilités car ils font partie de notre pile logicielle. Nous avons donc modifié le _hook_ FAI de la @fig:debian:syft:hook pour également scanner les images de conteneurs et générer des @SBOM:pl, qu'on exporte en même temps que celui du système de fichiers. Le pipeline pour donner tous ces @SBOM:pl à Grype et à VulnScout a également été modifié afin d'avoir une vue des vulnérabilités la plus complète possible.

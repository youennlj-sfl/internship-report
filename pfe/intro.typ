...
// La plupart des projets utilisent le projet Yocto @yoctoproject. C’est un ensemble d’outils pour configurer et compiler des distributions Linux pour l’embarqué, adaptées au matériel cible, avec les applications et programmes souhaités. Cela permet d’intégrer uniquement ce qui est utile, permettant notamment de réduire la taille des images générées. Toute la configuration se fait par un ensemble de scripts shell et python. Chaque élément à installer, comme un programme, correspond à une recipe, ou recette. On y spécifie comment récupérer les sources, comment compiler ou encore ce qui doit être installé. // TODO reduce

Pour collaborer efficacement, chaque projet est versionné dans des dépôts Git @git. Chez #sfl, l’application Gerrit @gerrit héberge les dépôts, et met en place un système de revue de code. Chaque commit est vérifié par une (ou plusieurs, selon les projets) autres personnes de l'entreprise, ce qui permet de détecter les erreurs ou de proposer de meilleures approches. Lorsque la personne faisant la revue demande ou propose des modifications, le commit en question doit être modifié, afin de garder un historique parfaitement cohérent. Un commit n’est ajouté dans la branche principale d’un dépôt que lorsque la revue est validée. Certains projets @opensource comme SEAPATH ou VulnScout utilisent plutôt GitHub @github, où le même principe de revue de code est appliqué.

L’intégration continue, ou @CI, ajoute un deuxième filet de sécurité. C’est un ensemble de services automatisés pour compiler et tester du code. L’entreprise utilise l’application Jenkins @jenkins pour les projets utilisant Gerrit, ou bien le système GitHub Actions @github-actions pour les autres. À chaque nouveau commit ou @PR, un cycle de @CI commence et va effectuer diverses tâches : compiler le projet, lancer des tests, etc. Cela augmente les chances de détecter une régression, de rapidement régler les bugs, et d’éviter une longue investigation de problème plus tard. Sur des projets applicatifs, des tests d’analyse statique du code permettent aussi de maintenir une bonne qualité de code.
...
- CQFD @cqfd, permet de facilement lancer des commandes dans un @conteneur ayant accès au dossier courant. Utilisé notamment pour configurer et utiliser le projet Yocto avec toutes les dépendances nécessaires, peu importe l’environnement d’exécution.
- Cukinia @cukinia, un outil de test pour vérifier la configuration d’un système Linux au runtime : présence d’utilisateurs, de partitions de système de fichier, de programmes, de périphériques... Il permet d’appliquer une méthodologie Test-Driven Development (TDD) pour l’intégration d’un système Linux complet.

Concernant l'organisation interne, les informations importantes sont diffusées par e-mails. Pour le quotidien, une instance Mattermost est utilisée comme plateforme de communication instantanée. La majorité du travail est faite en présentiel, mais il arrive de faire des réunions par visioconférence, notamment avec l'équipe de Montréal, notamment lorsque les deux bureaux travaillent sur des projets communs.
...
SEAPATH utilise le module noyau KVM @kivity2007kvm, permettant de convertir l’OS en un @hyperviseur dit "de type 1", ce qui signifie que les @VM:pl s'exécutent directement "sur le matériel". Chaque @VM est alors implémentée comme un processus Linux standard. De plus, SEAPATH utilise QEMU @qemu en conjonction avec KVM. Dans cette configuration, KVM se charge des interactions avec le matériel tandis que QEMU émule les périphériques et gère l’environnement de la @VM. Enfin, libvirt @libvirt est utilisé pour gérer et orchestrer les @VM:pl.
...
- *Open vSwitch* @openvswitch, une implémentation logicielle d’un switch ethernet.
- *Pacemaker* @pacemaker, chargé de démarrer, arrêter et superviser les ressources du cluster.
- *Corosync* @corosync, un système de communication de groupe.
- *Ceph* @ceph, un solution de stockage distribué.
...
Enfin, on utilise Ansible @hochstein2017ansible, un outil d’automatisation, pour configurer divers éléments de l’infrastructure déployée en utilisant des *playbooks*. Ces playbooks définissent l’état souhaité de l’infrastructure sous forme de code, décrivant les tâches à exécuter sur les différents nœuds du système, également appelés "hosts". Les nœuds sont décrits dans un *inventaire* qui est fourni au playbook.

//== Objectifs du PFE
=== Aspects de cybersécurité

Actuellement, le projet SEAPATH ne propose pas de moyen simple à ses utilisateurs pour suivre les vulnérabilités logicielles présentes dans le système. Les mainteneurs eux-même n'ont pas connaissance de celles-ci, ce qui est un grave problème, SEAPATH étant destiné à terme à être utilisé dans des infrastructures critiques.

De plus, le monde de la cybersécurité s'accélère depuis quelques années. Les vulnérabilités découvertes sont de plus en plus nombreuses, et les organisations étatiques et même internationales commencent à légiférer pour obliger les fournisseurs de solutions numériques à se préoccuper de ces questions. Par exemple, l'Union Européenne a créé le Cyber Resilience Act (CRA) @cra-article qui prendra effet en 2027, d'où l'intérêt pour les mainteneurs de SEAPATH d'agir maintenant.

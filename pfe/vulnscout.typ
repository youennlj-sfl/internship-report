...
#let vs-presentation = [
  VulnScout @vulnscout est un outil de scan et d'évaluation de vulnérabilités basé sur les @SBOM:pl. Il est fait pour être convivial pour les développeurs et simple à intégrer avec Yocto, Buildroot (un autre outil de génération d'images Linux embarquées) et avec la plupart des systèmes générant des SBOMs car il prend en charge tous les formats standards (SPDX, CycloneDX, OpenVEX). Il a été conçu pour être très facile à mettre en place avec le minimum de configuration nécessaire. Il comble ainsi le fossé entre les outils de cybersécurité, souvent complexes et difficiles à mettre en place, et le développement logiciel au quotidien, notamment dans les projets comportant des dizaines voire des centaines de dépendances open source.
]
/*#meander.reflow({
  import meander: *
...
Ces tâches ont principalement été menées sur la page "Vulnérabilités" de l'interface graphique (voir @fig:vulnscout:seapath:vulnerabilities) : en filtrant uniquement sur les vulnérabilités en attente et en les triant par ordre de sévérité, il est aisé de voir les détails des vulnérabilités critiques pour ensuite chercher si elles étaient applicables ou non (voir @fig:vulnscout:seapath:vuln-modal). Cela a été d'une grande aide : chercher directement dans les fichiers SPDX et OpenVEX était très complexe dû à leur grande verbosité.
...
VulnScout étant encore en bêta, il était normal d'y trouver quelques bugs ou fonctionnalités manquantes au fil de notre utilisation. On a donc remonté ceux-ci sur le dépôt de VulnScout. Quand ceux-ci étaient bloquants pour le stage, on les a directement réglés et proposé les contributions via des @PR:pl.

En effet, notre utilisation de VulnScout est légèrement différente de celle faite par les autres équipes de Savoir-faire Linux : là où celles-ci l'utilisent directement via _meta-vulnscout_, nous paramétrons et lançons l'outil en utilisant la ligne de commande. Ainsi, nous avons pu corriger des soucis en rapport avec cette différence : par exemple, l'import des fichiers SPDX générés par SEAPATH rencontrait des problèmes de permissions. De plus, comme indiqué au @chapter:debian:grype, nous ingérons le résultat d'un scan Grype directement dans VulnScout sans passer par l'outil de scan intégré, on a donc rajouté une nouvelle commande `--add-grype` à cet effet.
...
Comme le seuil à partir duquel une vulnérabilité est considérée comme critique utilise la valeur de l'@EPSS, VulnScout doit récupérer ces valeurs depuis la base de données en ligne, et ce pour toutes les vulnérabilités. Cette opération peut prendre plusieurs minutes selon la quantité de vulnérabilités détectées, il est donc préférable de ne pas la faire à chaque exécution de la @CI. Pour éviter cela, on a fait en sorte de garder la base de données de VulnScout en cache pour la réutiliser entre les exécutions.

...
On a créé 4 scénarios de test qu'on a lancés sur une même version de SEAPATH pour pouvoir comparer les résultats :
- scénario "basique" utilisant uniquement la classe `cve-check` de Yocto ;
...
Ainsi, après avoir filtré la centaine de vulnérabilités détectées avec les seuils énoncés précédemment, il n'est plus resté qu'une quinzaine de vulnérabilités à analyser, dont une majorité étaient des faux positifs :
- Certains étaient dus à la configuration de SEAPATH. Par exemple #link("https://www.cve.org/CVERecord?id=CVE-2026-32746")[CVE-2026-32746], une vulnérabilité dans le binaire _telnetd_ contenu dans le paquet _inetutils_, est détectée alors que SEAPATH contient une partie seulement d'_inetutils_ (comme _ping_ par exemple) mais pas _telnetd_.
...
Enfin, la documentation de SEAPATH Yocto a été mise à jour pour indiquer aux futurs contributeurs et utilisateurs comment effectuer cette analyse de vulnérabilités, en listant les fichiers à utiliser et les commandes à exécuter pour les traiter.

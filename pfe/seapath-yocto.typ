...
Après avoir établi une liste de solutions potentielles dans le @chapter:yocto:sota, il a fallu sélectionner celle la plus adaptée pour SEAPATH. Pour cela, on a testé chacune de ces solutions pour en tirer des évaluations quantitatives : on mesure le *nombre total de vulnérabilités* remontées, et on évalue le nombre de *faux positifs* (vulnérabilités annoncées comme applicables par les outils mais qui ne le sont pas) et de *faux négatifs* (vulnérabilités non reportées par les outils).
...
Déjà, on a pu remarquer un nombre total de vulnérabilités potentiellement applicables assez importante : environ 130. Toutes les analyser aurait potentiellement pu prendre plusieurs jours voire même semaines, on a donc décidé de les filtrer selon leur importance. On a utilisé pour cela les deux métriques courantes pour estimer l'importance de vulnérabilités : le @CVSS qui indique la sévérité et l'@EPSS qui indique l'exploitabilité. Une vulnérabilité avec un score de sévérité faible et une exploitabilité nulle ne vaut pas le coup d'être étudiée. La portée de notre analyse a donc été définie uniquement pour les vulnérabilités vérifiants une de ces conditions :
